...
class PatientTestRepository(BaseRepository):
    async def get_assigned_test_by_test_id_and_patient_id(self, test_id: UUID4, patient_id: UUID4) -> PatientTest | None:
    ...
    async def get_assigned_tests_by_patient_id(self, patient_id: UUID4) -> list[PatientTest]:
    ...
    async def get_assigned_patient_tests_by_doctor_id(self, doctor_id: UUID4, patient_id: UUID4) -> list[PatientTest]:
    ...
    async def get_test_by_patient_id(self, patient_id: UUID4, test_id: UUID4) -> PatientTest | None:
        return self.db.query(PatientTest).filter(
            PatientTest.id == test_id,
            PatientTest.patient_id == patient_id
        ).first()

    async def get_test_by_doctor_id_and_patient_id(self, test_id: UUID4, doctor_id: UUID4, patient_id: UUID4) -> PatientTest | None:
        return self.db.query(PatientTest).filter(
            PatientTest.patient_id == patient_id,
            PatientTest.id == test_id,
            DoctorPatient.doctor_id == doctor_id
    ...
    async def unassign_patient_tests_assigned_by_doctor(self, doctor_id: UUID4, patient_id: UUID4):
