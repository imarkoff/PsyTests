...
@router.get("/", summary="Get patient tests", responses={
...
@router.post("/{test_id}", summary="Assign test to patient", status_code=201, response_class=Response,
             responses={
                 404: {"description": "Test or patient not found"},
                 409: {"description": "Test already assigned to patient"},
                 201: {"description": "Test assigned to patient"}
             })
async def assign_test(
    ...
    try:
        await patient_tests_service.assign_test(db, test_id, doctor_id=doctor.id, patient_id=patient_id)
